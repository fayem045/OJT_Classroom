...
async function createTables() {
  ...
  try {
    ...
    await client.query(`
      ...
      CREATE TYPE activity_type AS ENUM ('student', 'professor', 'company', 'system');
      CREATE TYPE report_status AS ENUM ('pending', 'submitted', 'reviewed', 'approved', 'rejected');
      CREATE TYPE task_status AS ENUM ('pending', 'in_progress', 'completed', 'overdue');
    `).catch(err => {
      if (err.code === '42710') {
        console.log('ℹ️ Enums already exist');
      } else {
        throw err;
      }
    });
